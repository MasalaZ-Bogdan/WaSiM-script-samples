'''
This is an expansion of the script that automates WaSiM simulation runs. When assigning WaSiM to output 
rasters for snow depth, glacier cover, and glacier mass balance, the sizes of output folders can balloon 
pretty quickly. Including data processing steps directly after a simulation reduces output folder sizes.
Processing steps here include:
1. Computing fractional snow-covered area ("fsc")
2. Computing fractional glacier-covered area ("fgc")
3. Deleting all output files that don't have matching dates with observations
